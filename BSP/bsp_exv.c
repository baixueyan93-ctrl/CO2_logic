 ...
 *  每拍交替单相/双相通电, 8拍为一个完整周期
 *
 *  半步进相序 (1=通电, 0=断电, 与V13原理图标注一致):
 *    PM0A=B-, PM0B=A-, PM0C=B, PM0D=A
 *
 *    Step 0: B- + A-  (双相)
 *    Step 1: B-       (单相)
 *    Step 2: B- + A   (双相)
...
/* 相序表: 1-2相励磁半步进 (8拍)
 * 每个元素: 1=通电, 0=断电
 * 排列: {PM0A(B-), PM0B(A-), PM0C(B), PM0D(A)}  — 与V13原理图一致
 *
 * 单极电机线圈: COM(+12V) 分出 A/A-/B/B- 四个半绕组
 * 相序: B-+A- → B- → B-+A → A → B+A → B → B+A- → A- → (循环)
 */
static const uint8_t PHASE_TABLE[8][4] = {
    /* B-(A) A-(B) B(C)  A(D)    通电的半绕组     */
    {  1,    1,    0,    0  },  /* Step 0: B-+A- (双相) */
    {  1,    0,    0,    0  },  /* Step 1: B-    (单相) */
    {  1,    0,    0,    1  },  /* Step 2: B-+A  (双相) */
    {  0,    0,    0,    1  },  /* Step 3: A     (单相) */
    {  0,    0,    1,    1  },  /* Step 4: B+A   (双相) */
    {  0,    0,    1,    0  },  /* Step 5: B     (单相) */
    {  0,    1,    1,    0  },  /* Step 6: B+A-  (双相) */
    {  0,    1,    0,    0  },  /* Step 7: A-    (单相) */
};
