...
/* 相序表: 1-2相励磁半步进 (8拍)
 * 每个元素: 1=通电, 0=断电
 * 排列: {PM0A(相A), PM0B(相B), PM0C(相C), PM0D(相D)}
 *
 * 相序推导: 从原4拍全步进 AB→AD→DC→CB 插入单相中间步
 *           AB → A → AD → D → DC → C → CB → B → (AB...)
 */
static const uint8_t PHASE_TABLE[8][4] = {
    /* PM0A  PM0B  PM0C  PM0D    说明           */
    {  1,    1,    0,    0  },  /* Step 0: A+B (双相) */
    {  1,    0,    0,    0  },  /* Step 1: A   (单相) */
    {  1,    0,    0,    1  },  /* Step 2: A+D (双相) */
    {  0,    0,    0,    1  },  /* Step 3: D   (单相) */
    {  0,    0,    1,    1  },  /* Step 4: D+C (双相) */
    {  0,    0,    1,    0  },  /* Step 5: C   (单相) */
    {  0,    1,    1,    0  },  /* Step 6: C+B (双相) */
    {  0,    1,    0,    0  },  /* Step 7: B   (单相) */
};
